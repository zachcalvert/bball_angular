...
urlpatterns = [  
    ...
    url(r'^(?P<league_id>\d+)/matchup/(?P<matchup_id>\d+)?$', views.MatchupView.as_view(), name='matchup'),
    url(r'^(?P<league_id>\d+)/free_agents/?$', cache_page(60*60)(views.FreeAgentsView.as_view()), name='free_agents'),
    url(r'^(?P<league_id>\d+)/draft/results/', views.DraftResultsView.as_view(), name='draft_results'),
]